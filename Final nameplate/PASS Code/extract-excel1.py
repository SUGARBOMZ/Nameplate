...
def add_powtr_codes(results):
    for r in results:
        ...
        if isinstance(d, dict) and not any(k in d for k in ('error','raw_text')):
            ...
            if code.startswith("ไม่สามารถ"):  # หรือเงื่อนไขอื่น
                d['POWTR_CODE'] = ""
            else:
                d['POWTR_CODE'] = code
...
def split_value_unit(val: str):
    """แยก '12345 kg' → ('12345','kg'), ถ้าไม่มีหน่วย คืน (val,'')"""
    s = str(val).strip()
...
with tab1:
    ...
    if st.button("ประมวลผลภาพ", key="btn_extract") and excel_f1 and imgs:
        ...
        for r in results:
            ...
            # วนแต่ละ attribute ที่เหลือ
            for attr, raw_val in d.items():
                # ข้ามคอลัมน์ที่ไม่ใช่ข้อมูล attribute
                if attr in ("ASSETNUM", "SITEID", "POWTR_CODE"):
                    continue

                raw_str = str(raw_val).strip()
                # แยกหมายเลข กับ หน่วยวัด
                m = re.match(r"^(.*?)\s*([^\d\s][\w%°µΩ/+-]+)$", raw_str)
                if m:
                    val  = m.group(1).strip()
