...
def _kv_from_text(txt: str) -> float | None:
    """Extract highest KV value from text, ignore KVA/A, skip BIL/IMPULSE."""
    txt_u = txt.upper()
    best = None
    for chunk in re.split(r'[\/;,]', txt_u):
        if 'BIL' in chunk or 'IMPULSE' in chunk:
            continue
        if re.search(r'\bK?VA\b|\bKA\b|\bA\b|AMP', chunk):
            continue
...
def add_powtr_codes(results: list[dict]) -> list[dict]:
    """Append POWTR_CODE to each result dict."""
    for r in results:
...
def generate_prompt_from_excel(excel_file) -> str:
    """Build Thai prompt from attribute list in excel."""
    try:
    ...
    df.columns = [cols[0]] + cols[1:]
    attribute_col = df.columns[0]
    prompt = [
        "กรุณาสกัดข้อมูลทั้งหมดจากรูปภาพนี้และแสดงผลในรูปแบบ JSON ที่มีโครงสร้างชัดเจน ..."
    ]
    for i,row in df.iterrows():
        attr = str(row[attribute_col]).strip()
        if not attr or attr.lower().startswith('nan'):
            continue
        prompt.append(f"{i+1}: {attr}")
    prompt.append("หากไม่พบข้อมูลสำหรับ attribute ใด ให้ใส่ค่า - แทน")
    return "\n".join(prompt)


def extract_data_from_image(api_key: str, img_b64: str, mime: str, prompt: str) -> str:
    """Call Gemini API to extract JSON text."""
